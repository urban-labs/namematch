...
@profile
def process_geo_column(df, variable):
    ...
    if ncols == 2:
        ...
        if df.no_comma.sum() > 0:
            logger.debug(f'Setting {variable.name} variable to missing for '
...
@profile
def process_address_column(df, logger=None):
    ...
    if ncols == 3:
        ...
        # parse address
        parsed_df = np.vectorize(parse_address)(df[df.columns.tolist()[0]].values)
...
def process_check(s, variable):
    ...
    if variable.check == 'Numeric':
        ...
        if not_numeric.sum() > 0:
            logger.debug(f'Setting {s.name} variable to missing for {not_numeric.sum()} '
                         f'rows (fails check).')
        s.loc[not_numeric] = ''
        ...
        after = s.isnull().sum()
        if (before - after) > 0:
            logger.debug(f'Setting {s.name} variable to missing for '
...
def process_set_missing(s, set_missing_list):
    ...
    if (missing.sum() > 0):
        logger.debug(f'Setting {s.name} variable to missing for {missing.sum()} '
...
def process_drop(s, drop_list):
    ...
    if (len(records_to_drop) > 0):
        logger.debug(f'Adding {len(records_to_drop)} rows to drop list because '
...
@profile
def process_data(df, variables, data_file, params):
    ...
    for variable in variables.varlist:
        ...
        if (variable.compare_type == 'Geography'):
            relevant_df = process_geo_column(df[relevant_columns], variable)
        elif (variable.compare_type == 'Address'):
            relevant_df = process_address_column(df[relevant_columns])
        else:
...
class ProcessInputData(NamematchBase):
    def __init__(
        ...
        **kwargs
        ...
        super(ProcessInputData, self).__init__(params, schema, output_file, logger_id, *args, **kwargs)
    ...
    def main__process_input_data(self, quick_test=False, **kw):
        ...
        Args:
            ...
            output_file (str): path to output_temp's all-names file
            quick_test (bool): if True, only process one chunk of each input file
        '''
        ...
        if self.params.incremental:
            ...
            logger.stat('run_type: incremental')

        else:
            logger.info("This is not an incremental run.")
            logger.stat('run_type: "from scratch"')

        logger.debug("Start: process_input_data")
        ...
        logger.stat_dict({'categorical_variables': cat_vars})
        ...
        for j, data_file in enumerate(data_file_list):
            ...
            for i, df in enumerate(pd.read_csv(
                    data_file.filepath,
                    chunksize=self.input_data_batch_size,
                ...
                if len(df) == 0:
                    break
                if quick_test and (data_file.file_type == 'new') and (i == 1):
                    break

                df = df.fillna('')

                if self.params.verbose != None:
                    logger.info(f'  Processing: read {(i+1) * self.input_data_batch_size} rows.')

                df = process_data(df, self.schema.variables, data_file, self.params)

                first_iteration = (os.path.exists(self.output_file) == False)
                if len(df) > 0:
                    n_an_rows += len(df)
                    ...
                    if i == 0 and j == 0:
                        pqwriter = pq.ParquetWriter(self.output_file, table.schema)
